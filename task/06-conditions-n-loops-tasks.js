...
function isInsideCircle(circle, point) {
    let newx = Math.abs(circle.center.x - point.x);
    let newy = Math.abs(circle.center.y - point.y);
    if (Math.hypot(newx, newy) < circle.radius) 
        return true; 
    return false;
}
...
function reverseString(str) {
    let joinarr = str.split("");
    return joinarr.reverse().join("");
}
...
function isCreditCardNumber(ccn) {
    ...
    for(let i = 1; i <= StrNum.length; i++){
        let x = Number(StrNum[StrNum.length - i]);
        if (i % 2 === 0)
            x*=2;
        if (x > 9) 
           x -= 9;
...
function timespanToHumanString(startDate, endDate) {
    let val = (endDate - startDate) / 1000;
    if(val <= 45) 
        return 'a few seconds ago';
    if(val <= 90) 
        return 'a minute ago';
    if(val <= 45 * 60) 
        return Math.round((val - 0.001) / 60) + ' minutes ago';
    if(val <= 90 * 60) 
        return 'an hour ago';
    if(val <= 22 * 60 * 60) 
        return Math.round((val - 0.001) / (60 * 60)) + ' hours ago';
    if(val <= 36 * 60 * 60) 
        return 'a day ago';
    if(val <= 25 * 24 * 60 * 60) 
        return Math.round((val - 0.001) / (24 * 60 * 60)) + ' days ago';
    if(val <= 45 * 24 * 60 * 60) 
        return 'a month ago';
    if(val <= 345 * 24 * 60 * 60) 
        return Math.round((val - 0.001) / (30 * 24 * 60 * 60)) + ' months ago';
    if(val <= 545 * 24 * 60 * 60) 
        return 'a year ago';
    if(val >= 545 * 24 * 60 * 60 ) 
        return Math.round((val - 0.001) / (365 * 24 * 60 * 60)) + ' years ago';
}
